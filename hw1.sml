...
(* for a negative number, either 0-6 or ~6 *)
(* test for equality, use = instead of == in other languages *)
(* there is no assignment, just binding *)
(* "if then else" is a complete expression, "if then" is not. *)
...
(* andalso orelse is not just logical operator *)
