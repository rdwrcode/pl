(* sample after homework submitted *)
...
(* a better alternative avoiding isSome and valOf *)
...
fun remove_duplicates (xs: int list) =
  if null xs
  then []
  else let val tl_ans = remove_duplicates(tl xs)
       in
	   if mem(hd xs, tl_ans)
	   then tl_ans
	   else (hd xs)::tl_ans
...
fun reasonable_date (date: int*int*int) =
  ...
  in
      ...
      andalso day >= 1
      andalso day <= get_nth(lengths, month)
  end;
